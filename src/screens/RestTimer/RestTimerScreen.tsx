...
export function RestTimerScreen({ exerciseName = 'Bench Press', setNumber = 2, onClose }: Props) {
  ...
  const toggle = () => {
    if (done) { startPreset(total); return; }
    setRunning(r => !r);
  };
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={s.timerWrap}>
          <View style={s.ringWrap}>
            <Svg width={260} height={260} viewBox="0 0 260 260">
              ...
              <Circle cx={130} cy={130} r={R} fill="none" stroke={urgent ? 'rgba(240,80,80,0.20)' : 'rgba(123,94,250,0.16)'}
          ...
          <GlassView radius={999} style={s.nextPill}>
            <Text style={s.nextText}>Next: Set {setNumber + 1} of 4</Text>
          </GlassView>
          ...
          <View style={s.presets}>
            {PRESETS.map(p => (
              <TouchableOpacity key={p} onPress={() => startPreset(p)} activeOpacity={0.8}
                ...
                {seconds === p && !done
                  ? <LinearGradient colors={GRAD.accent} start={{x:0,y:0}} end={{x:1,y:1}} style={s.presetGrad}><Text style={s.presetTextActive}>{p < 60 ? p + 's' : Math.floor(p/60) + 'min'}</Text></LinearGradient>
        ...
        <View style={s.controls}>
          <TouchableOpacity onPress={skip} style={s.skipBtn} activeOpacity={0.8}>
            <GlassView radius={14} style={s.skipInner}><Text style={s.skipText}>Skip</Text></GlassView>
          </TouchableOpacity>
          <TouchableOpacity onPress={toggle} style={s.mainBtn} activeOpacity={0.9}>
            {(!running || done)
              ? <LinearGradient colors={GRAD.accent} start={{x:0,y:0}} end={{x:1,y:1}} style={s.mainGrad}><Text style={s.mainText}>{done ? 'Start Next Set' : 'Resume'}</Text></LinearGradient>
              : <GlassView opacity="high" radius={14} style={s.mainGlass}><Text style={s.pauseText}>Pause</Text></GlassView>}
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    </View>
...
const s = StyleSheet.create({
  ...
  timerWrap:       { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 20, paddingBottom: 20 },
  ringWrap:        { position: 'relative', width: 260, height: 260, alignItems: 'center', justifyContent: 'center' },
  ringCenter:      { position: 'absolute', alignItems: 'center' },
  doneCheck:       { fontSize: 52, color: COLORS.accent, lineHeight: 60 },
  ...
  nextPill:        { paddingHorizontal: 16, paddingVertical: 7, borderRadius: 999 },
